 ...
 * @since : 2018/9/29
 */
public class GridSectionAverageUItemDecoration extends RecyclerView.ItemDecoration {
     ...
     */
    public GridSectionAverageUItemDecoration(float gapHorizontalDp, float gapVerticalDp, float sectionEdgeHPaddingDp, float sectionEdgeVPaddingDp) {
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        if (parent.getLayoutManager() instanceof GridLayoutManager && parent.getAdapter() instanceof BaseSectionQuickAdapter) {
            ...
            OilArrayBean entity = (OilArrayBean) adapter.getItem(position);
...
//            Log.w("GridAverageGapItem", "pos=" + position + ",vPos=" + visualPos + "," + outRect.toShortString());
    ...
    private void markSections() {
        if (mAdapter != null) {
            ...
            mSectionList.clear();
            OilArrayBean sectionEntity = null;
            Section section = new Section();
            for (int i = 0, size = adapter.getItemCount(); i < size; i++) {
                sectionEntity = (OilArrayBean) adapter.getItem(i);
