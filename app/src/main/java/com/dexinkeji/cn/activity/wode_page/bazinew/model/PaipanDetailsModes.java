package com.dexinkeji.cn.activity.wode_page.bazinew.model;

import java.util.List;

public class PaipanDetailsModes {
    private String shiErGong;
    private List<String> xing;
    private List<String> xing_se;
    private String xiaoXian;
    private String daXian;

    private String boShi;
    private String jiangQian;
    private String suiQian;
    private String wuXingChangSheng;
    private String shiErGongNaYin;
    private String shiErGongTianGan;


    public String getShiErGong() {
        return shiErGong;
    }

    public void setShiErGong(String shiErGong) {
        this.shiErGong = shiErGong;
    }

    public List<String> getXing() {
        return xing;
    }

    public void setXing(List<String> xing) {
        this.xing = xing;
    }

    public List<String> getXing_se() {
        return xing_se;
    }

    public void setXing_se(List<String> xing_se) {
        this.xing_se = xing_se;
    }

    public String getXiaoXian() {
        return xiaoXian;
    }

    public void setXiaoXian(String xiaoXian) {
        this.xiaoXian = xiaoXian;
    }

    public String getDaXian() {
        return daXian;
    }

    public void setDaXian(String daXian) {
        this.daXian = daXian;
    }

    public String getBoShi() {
        return boShi;
    }

    public void setBoShi(String boShi) {
        this.boShi = boShi;
    }

    public String getJiangQian() {
        return jiangQian;
    }

    public void setJiangQian(String jiangQian) {
        this.jiangQian = jiangQian;
    }

    public String getSuiQian() {
        return suiQian;
    }

    public void setSuiQian(String suiQian) {
        this.suiQian = suiQian;
    }

    public String getWuXingChangSheng() {
        return wuXingChangSheng;
    }

    public void setWuXingChangSheng(String wuXingChangSheng) {
        this.wuXingChangSheng = wuXingChangSheng;
    }

    public String getShiErGongNaYin() {
        return shiErGongNaYin;
    }

    public void setShiErGongNaYin(String shiErGongNaYin) {
        this.shiErGongNaYin = shiErGongNaYin;
    }

    public String getShiErGongTianGan() {
        return shiErGongTianGan;
    }

    public void setShiErGongTianGan(String shiErGongTianGan) {
        this.shiErGongTianGan = shiErGongTianGan;
    }
}
