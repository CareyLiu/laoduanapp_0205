...
public class BaseViewHolder extends RecyclerView.ViewHolder {
    ...
    /**
     * 去空判断 -- 第三个参数是为空的时候的默认值，不需要显示的时候传空
     *
     * @param viewId
     * @param value  字段赋值
     * @param str    默认值写可不写
     * @return
     */
    public BaseViewHolder mfSetText(@IdRes int viewId, CharSequence value, String str) {
        TextView view = getView(viewId);
    ...
    /**
     * 去空判断 -- 第三个参数是为空的时候的默认值，不需要显示的时候传空 在之前的基础上 添加针对需要.00的
     *
     * @param viewId
     * @param value  字段赋值
     * @param str    默认值写可不写
     * @return
     */
    public BaseViewHolder mfSetNumberText(@IdRes int viewId, String value, String str) {
        TextView view = getView(viewId);
        if (!TextUtils.isEmpty(value)) {
            view.setText(NumberFormat.getCurrencyInstance().format(Double.valueOf(value)));
        } else {
            if (!TextUtils.isEmpty(str)) {
                view.setText(str);
            } else {
     ...
     * @param size   大小
     * @param flag   字体是否加粗  true 加粗 false 不加
     * @return
     */
    public BaseViewHolder setTextSize(@IdRes int viewId, int type, int size, boolean flag) {
        TextView view = getView(viewId);
        view.setTextSize(type, size);
        if (flag) {
    ...
    public BaseViewHolder append(@IdRes int viewId, CharSequence value) {
        TextView view = getView(viewId);
        view.append(value);
        return this;
    }
     ...
     * @return
     */
    public BaseViewHolder addRxDoubleOnClickListener(@IdRes final int viewId) {
        childClickViewIds.add(viewId);
        final View view = getView(viewId);
        if (view != null) {
            if (!view.isClickable()) {
    ...
    public BaseViewHolder addOnClickListener(@IdRes final int viewId, @IdRes final int mainViewId) {
        childClickViewIds.add(viewId);
        final View view = getView(viewId);

        final View mainView = getView(mainViewId);
        if (view != null) {
            if (!view.isClickable()) {
                view.setClickable(true);
            }
            view.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    if (adapter.getMfOnItemChildClickListener() != null) {
                        adapter.getMfOnItemChildClickListener().mfOnItemChildClick(adapter, v, mainView, getClickPosition());
                    }
                }
            });
        }
    ...
    public void setGravity(@IdRes int viewId, int gravity) {
        CheckedTextView checkedTextView = getView(viewId);
